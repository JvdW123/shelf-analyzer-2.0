...
For MULTI-PACKS (e.g., box of 10 shots): Count the multi-pack box as 1 SKU with 1 facing. The box is the unit. Do not count individual items inside the box.
...
If you see a price tag with no product above/below it (empty gap, dark space, or clearly vacant slot), record the SKU using the price tag information and mark Stock Status as "Out of Stock". Use the tag's text to fill in brand, product name, price, and size where readable.
...
| Packaging Size    | Price tag          | Product label           | Leave blank (null) |
...
20 Packaging Size (ml) | Price tag (primary), product label (secondary) | Volume in milliliters (e.g., 100, 250, 330, 750, 900, 1000, 1500). Check the price tag first — UK tags often show the volume. Verify against the product label if visible. Leave blank if not visible on either source. Do NOT estimate.
